...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and prints the URL with the LAN snapshot', async () => {
    ...
    apply(ctx, new Config({ printUrl: true, surfaceContext: true, trustedHosts: ['lab.internal'], trustedOrigins: ['https://lab.internal'], dev: false }))
    ...
    expect(ctx.get('webRuntime')).toEqual({
      ...
      trustedOrigins: ['https://lab.internal'],
      dev: false,
    })
  ...
  it('stays quiet with printUrl off', async () => {
    ...
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ printUrl: false, surfaceContext: true, trustedHosts: [], trustedOrigins: [], dev: false }))
    await ctx.plugin(SystemPrompt, { persona: '' })
    await new Promise(resolve => setTimeout(resolve, 0))
  ...
  it('skips the surface context when disabled (the one-shot layer): no prompt section, no bash variables', async () => {
    ...
    apply(ctx, new Config({ printUrl: false, surfaceContext: false, trustedHosts: [], trustedOrigins: [], dev: false }))
    await ctx.plugin(SystemPrompt, { persona: '' })
    await new Promise(resolve => setTimeout(resolve, 0))
    const assembly = await ctx.systemPrompt.assemble()
  ...
  it('prints the loopback-only URL line when no LAN snapshot exists', async () => {
    ...
    ctx.provide('webServer', fakeHttpServer().server)
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(ctx, new Config({ printUrl: true, surfaceContext: true, trustedHosts: [], trustedOrigins: [], dev: false }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).toHaveBeenCalledWith('dsh web: http://127.0.0.1:4567')
    await ctx.fiber.dispose()
  })

  it('warns once when --dev disables the browser-trust fences', async () => {
    stageDist()
    const ctx = new Context()
    ctx.provide('webServer', fakeHttpServer().server)
    const warn = vi.spyOn(ctx.logger, 'warn').mockImplementation(() => {})
    apply(ctx, new Config({ printUrl: false, surfaceContext: false, trustedHosts: [], trustedOrigins: [], dev: true }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(ctx.get('webRuntime')).toMatchObject({ dev: true })
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('--dev is active'))
    await ctx.fiber.dispose()
  })
  ...
  it('defers the URL line until Loader settlement and drops it on failure or teardown', async () => {
    ...
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    apply(settled, new Config({ printUrl: true, surfaceContext: true, trustedHosts: [], trustedOrigins: [], dev: false }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).not.toHaveBeenCalled()
    release!()
    await new Promise(resolve => setTimeout(resolve, 0))
    ...
    apply(failed, new Config({ printUrl: true, surfaceContext: true, trustedHosts: [], trustedOrigins: [], dev: false }))
    await new Promise(resolve => setTimeout(resolve, 0))
    expect(log).not.toHaveBeenCalled()
    ...
    apply(torn, new Config({ printUrl: true, surfaceContext: true, trustedHosts: [], trustedOrigins: [], dev: false }))

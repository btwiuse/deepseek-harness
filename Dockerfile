...
# command, and starts `dsh web --dev --host 0.0.0.0` directly: no entrypoint
# wrapper, because the CLI maps $PORT to the listen port, --host 0.0.0.0 binds
# every interface so the platform proxy (e.g. Railway) can reach the container,
# and --dev disables every /api browser-trust fence.
